...
import { categoriesIcons } from '../components/storage/categories'
...
export default function Task({ task }) {
  ...
  const { description, image } = categoriesIcons
    .find(({ description }) => description === category)
   ...
   * @param {object} taskHTML - объект с HTML элементами задания и кнопки "Завершить" 
   */
  const setTaskCompletion = (taskHTML) => {
    const { taskElement, buttonComplete } = taskHTML

    if (task.completed) {
      taskElement.classList.add('completed')
      buttonComplete.innerText = 'Undone'
    } else {
